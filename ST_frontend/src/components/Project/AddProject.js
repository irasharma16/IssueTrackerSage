...
const AddProjects = () => {
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            ...
            if (modalMode === 'edit') {
                await axios.put(`http://localhost:8080/Project/${formData.projectCode}`, formDataToSend, {
                    headers: { 'Content-Type': 'multipart/form-data' }
                });
                toast.success('Project updated successfully!');
            } else {
                await axios.post('http://localhost:8080/Project', formDataToSend, {
                    headers: { 'Content-Type': 'multipart/form-data' }
                });
    ...
    const handleDelete = async () => {
        if (window.confirm('Are you sure you want to delete this project?')) {
            try {
                await axios.delete(`http://localhost:8080/Project/${formData.projectCode}`);
