...
function TimeSheet() {
  ...
  const fetchData = async () => {
    try {
      const response = await axios.get('http://localhost:8080/timesheet');
      ...
      try {
        ...
        if (selectedRow !== null) {
          response = await axios.put(
            `http://localhost:8080/timesheet/${submittedData[selectedRow]._id}`,
            formData,
          ...
          response = await axios.post('http://localhost:8080/timesheet', formData, {
            headers: { 'Content-Type': 'multipart/form-data' },
          });
                ...
                <td>
                  {entry.uploadedFile ? (
                    <>
                      {/* View File */}
                      <a
                        href={`http://localhost:8080/uploads/timesheets/${entry.uploadedFile}`}
                        target="_blank"
                      ...
                      {/* Download File */}
                      <a
                        href={`http://localhost:8080/download/${entry.uploadedFile}`}
                        download
