...
const Register = () => {
  ...
  const fetchData = async (url) => {
    try {
      ...
      if (userRole === 'client' && url.includes('registerissue')) {
        ...
        const userResponse = await axios.get('http://localhost:8080/userlist/users');
  ...
  useEffect(() => {
    const loadData = async () => {
      const registerissue = await fetchData('http://localhost:8080/registerissue');
      ...
      setClients(await fetchData('http://localhost:8080/client'));
      setIssueTypes(await fetchData('http://localhost:8080/issues'));
      setStatuses(await fetchData('http://localhost:8080/status'));
      setModules(await fetchData('http://localhost:8080/modules'));
      setEmployees(await fetchData('http://localhost:8080/employees'));
      setSizes(await fetchData('http://localhost:8080/size'));
      setResolutionTypes(await fetchData('http://localhost:8080/resolutionTypes'));
      setLevels(await fetchData('http://localhost:8080/levels'));
      setPriorities(await fetchData('http://localhost:8080/priority'));
    };
    ...
    try {
      ...
      if (modalMode === 'edit') {
        const response = await axios.put(
          `http://localhost:8080/registerissue/${formData.issueNumber}`,
          formData,
          {
        ...
        const response = await axios.post(
          'http://localhost:8080/registerissue',
          formData,
          {
            params: { userRole, loggedInUser, company }
          }
  ...
  const handleDelete = async () => {
    try {
      ...
      await axios.delete(`http://localhost:8080/registerissue/${issueNumber}`, {
        params: { userRole }
      });
