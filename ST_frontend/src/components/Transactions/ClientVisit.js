...
function ClientVisit() {
  ...
  const fetchData = async () => {
    try {
      ...
      const response = await axios.get('http://localhost:8080/clientvisit');
  ...
  const handleSave = async () => {
    if (assignTo && client && purpose && module && contactPerson && visitDateFrom && visitDateTo && timeEstimate && remark) {
      ...
      try {
        let response;
        if (selectedRow !== null) {
          response = await axios.put(`http://localhost:8080/clientvisit/${clientVisits[selectedRow]._id}`, visitData);
        } else {
          response = await axios.post('http://localhost:8080/clientvisit', visitData);
        }
