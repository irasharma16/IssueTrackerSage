...
function Team() {
  ...
  const handleSave = () => {
    if (teamName) {
      ...
      if (isEditMode && selectedRow !== null) {
        axios.put(`http://localhost:8080/teams/update/${teams[selectedRow]._id}`, newOrUpdateTeam)
          .then(response => {
        ...
        axios.post('http://localhost:8080/teams', newOrUpdateTeam)
          .then(response => {
  ...
  const handleDelete = () => {
    if (selectedRow !== null) {
      axios.put(`http://localhost:8080/teams/softDelete/${teams[selectedRow]._id}`)
        .then(() => {
