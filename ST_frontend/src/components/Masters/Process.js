...
function Process() {
  ...
  const handleSaveOrUpdate = () => {
    if (processName) {
      ...
      if (isUpdate && selectedRow !== null) {
        axios.put(`http://localhost:8080/process/update/${processes[selectedRow]._id}`, newOrUpdateProcess)
          .then(response => {
        ...
        axios.post('http://localhost:8080/process', newOrUpdateProcess)
          .then(response => {
  ...
  const handleDelete = () => {
    if (selectedRow !== null) {
      axios.put(`http://localhost:8080/process/softDelete/${processes[selectedRow]._id}`)
        .then(() => {
