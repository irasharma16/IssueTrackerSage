...
function EmployeeMaster() {
  ...
  const handleSave = () => {
    if (employeeName) {
      ...
      if (isEditMode && selectedRow !== null) {
        axios.put(`http://localhost:8080/employees/update/${employees[selectedRow]._id}`, newOrUpdateEmployee)
          .then(response => {
        ...
        axios.post('http://localhost:8080/employees', newOrUpdateEmployee)
          .then(response => {
  ...
  const handleDelete = () => {
    if (selectedRow !== null) {
      axios.put(`http://localhost:8080/employees/softDelete/${employees[selectedRow]._id}`)
        .then(() => {
