...
function Phase() {
  ...
  const handleSave = () => {
    ...
    if (isEditMode && selectedRow !== null) {
      ...
      axios.put(`http://localhost:8080/phase/update/${phases[selectedRow]._id}`, newOrUpdatePhase)
        .then(response => {
      ...
      // Save new phase
      axios.post('http://localhost:8080/phase', newOrUpdatePhase)
        .then(response => {
  ...
  const handleDelete = () => {
    if (selectedRow !== null) {
      axios.put(`http://localhost:8080/phase/softDelete/${phases[selectedRow]._id}`)
        .then(() => {
