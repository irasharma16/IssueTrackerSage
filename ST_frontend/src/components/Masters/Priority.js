...
function Priority() {
  ...
  const handleSave = () => {
    if (priorityName) {
      ...
      if (isEditMode && selectedRow !== null) {
        axios.put(`http://localhost:8080/priority/update/${priorities[selectedRow]._id}`, newOrUpdatePriority)
          .then(response => {
        ...
        axios.post('http://localhost:8080/priority', newOrUpdatePriority)
          .then(response => {
  ...
  const handleDelete = () => {
    if (selectedRow !== null) {
      axios.put(`http://localhost:8080/priority/softDelete/${priorities[selectedRow]._id}`)
        .then(() => {
