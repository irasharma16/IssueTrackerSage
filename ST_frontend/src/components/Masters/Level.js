...
function Level() {
  ...
  const handleSave = () => {
    if (levelName) {
      ...
      if (isEditMode && selectedRow !== null) {
        axios.put(`http://localhost:8080/levels/update/${levels[selectedRow]._id}`, newOrUpdateLevel)
          .then(response => {
        ...
        axios.post('http://localhost:8080/levels', newOrUpdateLevel)
          .then(response => {
  ...
  const handleDelete = () => {
    if (selectedRow !== null) {
      axios.put(`http://localhost:8080/levels/softDelete/${levels[selectedRow]._id}`)
        .then(() => {
