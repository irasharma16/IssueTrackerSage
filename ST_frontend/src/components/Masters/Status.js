...
function Status() {
  ...
  const handleSaveOrUpdate = () => {
    ...
    if (isEditing && selectedRow !== null) {
      ...
      axios.put(`http://localhost:8080/status/${statuses[selectedRow]._id}`, statusData)
        .then(response => {
          setStatuses(response.data);
      ...
      // Add new status
      axios.post('http://localhost:8080/status', statusData)
        .then(response => {
          setStatuses(response.data);
  ...
  const handleDelete = () => {
    if (selectedRow !== null) {
      axios.put(`http://localhost:8080/status/softDelete/${statuses[selectedRow]._id}`)
        .then(response => {
          setStatuses(response.data);
