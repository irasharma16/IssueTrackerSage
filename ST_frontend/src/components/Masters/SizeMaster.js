...
function SizeMaster() {
  ...
  const handleSave = () => {
    ...
    if (isEditMode) {
      axios.put(`http://localhost:8080/size/${sizes[selectedRow]._id}`, sizeData)
        .then(response => {
      ...
      axios.post('http://localhost:8080/size', sizeData)
        .then(response => {
  ...
  const handleDelete = () => {
    if (selectedRow !== null) {
      axios.put(`http://localhost:8080/size/softDelete/${sizes[selectedRow]._id}`)
        .then(() => {
