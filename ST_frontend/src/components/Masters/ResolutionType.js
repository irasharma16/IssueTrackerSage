...
function ResolutionType() {
  ...
  const fetchResolutionTypes = () => {
    setIsLoading(true);
    axios.get('http://localhost:8080/resolutionTypes')
      .then(response => {
  ...
  const handleSave = () => {
    if (resolutionTypeName) {
      ...
      if (isEditMode && selectedRow !== null) {
        axios.put(`http://localhost:8080/resolutionTypes/update/${resolutionTypes[selectedRow]._id}`, newOrUpdateResolutionType)
          .then(response => {
        ...
        axios.post('http://localhost:8080/resolutionTypes', newOrUpdateResolutionType)
  .then(response => {
  ...
  const handleDelete = () => {
    if (selectedRow !== null) {
      axios.put(`http://localhost:8080/resolutionTypes/softDelete/${resolutionTypes[selectedRow]._id}`)
        .then(() => {
