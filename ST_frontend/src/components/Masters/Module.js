...
function Module() {
  ...
  const handleSave = () => {
    if (moduleName) {
      ...
      if (isEditMode && selectedRow !== null) {
        axios.put(`http://localhost:8080/modules/update/${modules[selectedRow]._id}`, newOrUpdateModule, config)
          .then(response => {
        ...
        axios.post('http://localhost:8080/modules', newOrUpdateModule, config)
          .then(response => {
  ...
  const handleDelete = () => {
    if (selectedRow !== null) {
      axios.put(`http://localhost:8080/modules/softDelete/${modules[selectedRow]._id}`)
        .then(() => {
