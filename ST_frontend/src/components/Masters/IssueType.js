...
function IssueType() {
  ...
  const handleSave = () => {
    if (issueName) {
      ...
      if (isEditMode && selectedRow !== null) {
        axios.put(`http://localhost:8080/issues/update/${issues[selectedRow]._id}`, newOrUpdateIssue)
          .then(response => {
        ...
        axios.post('http://localhost:8080/issues', newOrUpdateIssue)
          .then(response => {
  ...
  const handleDelete = () => {
    if (selectedRow !== null) {
      axios.put(`http://localhost:8080/issues/softDelete/${issues[selectedRow]._id}`)
        .then(() => {
