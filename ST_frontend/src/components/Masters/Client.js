...
const Client = () => {
    ...
    const saveClient = async () => {
        ...
        try {
            if (selectedClient) {
                // Update client
                await axios.put(`http://localhost:8080/client/${selectedClient._id}`, data, {
                    headers: { 'Content-Type': 'multipart/form-data' }
                });
            } else {
                // Add new client
                await axios.post('http://localhost:8080/client', data, {
                    headers: { 'Content-Type': 'multipart/form-data' }
                });
            }
    ...
    const deleteClient = async () => {
        if (selectedClient && selectedClient.clientCode) {
            try {
                await axios.delete(`http://localhost:8080/client/${selectedClient.clientCode}`);
                            ...
                            clients.map((client, index) => (
                                <tr key={client._id}>
                                    ...
                                    <td>
                                        {client.companyLogo && (
                                            <img
                                                src={`http://localhost:8080/${client.companyLogo}`}
                                                alt="Company Logo"
