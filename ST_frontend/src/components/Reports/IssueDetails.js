...
const IssueDetails = () => {
    ...
    const fetchDataFromAPIs = async () => {
      ...
      try {
        const [
          ...
          fetchData('http://localhost:8080/client'),
          fetchData('http://localhost:8080/issues'),
          fetchData('http://localhost:8080/status'),
          fetchData('http://localhost:8080/modules'),
          fetchData('http://localhost:8080/employees'),
          fetchData('http://localhost:8080/size'),
          fetchData('http://localhost:8080/resolutionTypes'),
          fetchData('http://localhost:8080/levels')
        ]);
  ...
  const handleSearch = async () => {
    ...
    try {
      const userRole = localStorage.getItem('userRole');
      const loggedInUser = localStorage.getItem('loggedInUser');
      const url = 'http://localhost:8080/registerissue';
  ...
  const handleSubmit = async (e) => {
    ...
    setIsLoading(true);
    try {
      await axios.post('http://localhost:8080/registerissue', formData);
